...
function chooseBotMove() {
  ...
  for (const move of moves) {
    ...
    const recentPenalty = state.botRecentTiles.some((tile) => tile.row === move.row && tile.col === move.col) ? 180 : 0;
    const sameAxisPressure = (move.row === playerPos.row || move.col === playerPos.col) ? 55 : 0;

    const score =
      (winsNow ? 5000 : 0) +
      (adjacentNow ? 700 : 0) +
      (10 - Math.min(distance, 10)) * 55 +
