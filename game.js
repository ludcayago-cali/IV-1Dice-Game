...
function randomizeSpawns() {
  ...
  // Real randomness:
  // 1) pick Player 1 from any open tile
  // 2) pick Bot from open tiles that are not too close
  // 3) if that pool is empty, fall back to the farthest available tile
  const p1 = opens[randInt(opens.length)];
  const minSpawnDistance = 4;

  let candidates = opens.filter(tile =>
    !(tile.row === p1.row && tile.col === p1.col) &&
    Math.abs(tile.row - p1.row) + Math.abs(tile.col - p1.col) >= minSpawnDistance
  );
  ...
  if (!candidates.length) {
    ...
    for (const tile of opens) {
      if (tile.row === p1.row && tile.col === p1.col) continue;
      const d = Math.abs(tile.row - p1.row) + Math.abs(tile.col - p1.col);
      if (d > bestDistance) {
        bestDistance = d;
        farthest = [tile];
  ...
  const p2 = candidates[randInt(candidates.length)];
