...
const isMultiplayer = SOLO_ONLY ? false : (!!roomId && !!playerRole);
...
const MAP_01 = {
  name: "Crossroads Compact",
  startP1: { row: 0, col: 0 },
  startP2: { row: 5, col: 5 },

  ground: [
    ["grass1", "grass2", "grass1", "grass2", "grass1", "grass1"],
    ["grass2", "grass1", "grass2", "grass1", "grass2", "grass1"],
    ["grass1", "floor2", "grass1", "grass2", "grass1", "floor2"],
    ["grass2", "grass1", "floor2", "grass1", "floor1", "grass2"],
    ["floor1", "floor2", "floor1", "floor2", "floor1", "floor1"],
  ...
  blocks: [
    ...
    ["stone",   null,      "bonfire", null,      "stone",   null],
    [null,      null,      null,      "sand",    null,      null],
...
function getResponsiveTileSize() {
  ...
  const usableWidth = Math.min(viewportWidth - 64, 520);
  const totalGap = TILE_GAP * (SIZE - 1);
  return Math.max(40, Math.min(64, Math.floor((usableWidth - totalGap) / SIZE)));
}

function getBoardPixelSize() {
  return (SIZE * TILE_SIZE) + (TILE_GAP * (SIZE - 1));
}

function injectMobilePolishStyles() {
  if (document.getElementById("jarvis-mobile-polish")) return;

  const style = document.createElement("style");
  style.id = "jarvis-mobile-polish";
  style.textContent = `
    #board {
      position: relative;
      width: ${getBoardPixelSize()}px;
      height: ${getBoardPixelSize()}px;
      margin: 0 auto;
    ...
    .layer.block { z-index: 3; }
    .layer.fx { z-index: 4; }
    .layer.actor { z-index: 6; }
    .layer.actor.p1, .layer.actor.p2 {
      transform: scale(1.02);
      filter: drop-shadow(0 4px 6px rgba(0,0,0,.28));
    }
    .layer.actor.move {
      animation: jarvisHop .18s ease-out;
    }
    @keyframes jarvisHop {
      0% { transform: translateY(8px) scale(.98); opacity: .82; }
    ...
    .layer.fx.selected {
      box-shadow: inset 0 0 0 2px rgba(255,217,102,.95), 0 0 10px rgba(255,217,102,.45);
      border-radius: 10px;
    }
    @media (max-width: 600px) {
      ...
      #p1ScoreCard, #p2ScoreCard, .score-card, .player-card {
...
function refreshMobilePolishStyles() {
  const style = document.getElementById("jarvis-mobile-polish");
  if (!style) return;
  style.remove();
...
function updateBoardMetrics() {
  TILE_SIZE = getResponsiveTileSize();
  refreshMobilePolishStyles();

  if (boardEl) {
    boardEl.style.width = `${getBoardPixelSize()}px`;
    boardEl.style.height = `${getBoardPixelSize()}px`;
  }

  const wrap = boardEl?.parentElement;
  if (wrap) {
    wrap.style.width = "100%";
    wrap.style.maxWidth = `${getBoardPixelSize() + 12}px`;
...
async function animateStep(player, row, col) {
  ...
  if (player === 2) {
    state.botRecentTiles.unshift({ row, col });
    state.botRecentTiles = state.botRecentTiles.slice(0, 4);
  }
...
function chooseBotMove() {
  ...
  for (const move of moves) {
    ...
    const centerBias = 6 - (Math.abs(move.row - center) + Math.abs(move.col - center));
    const recentPenalty = state.botRecentTiles.some((tile) => tile.row === move.row && tile.col === move.col) ? 220 : 0;
    const sameAxisPressure = (move.row === playerPos.row || move.col === playerPos.col) ? 50 : 0;

    const score =
      (winsNow ? 7000 : 0) +
      (adjacentNow ? 900 : 0) +
      (10 - Math.min(distance, 10)) * 70 +
      (4 - Math.min(humanEscapes, 4)) * 100 +
      (centerBias * 16) +
      sameAxisPressure -
      recentPenalty +
...
function getAllOpenTiles(map = MAP_01) {
  const tiles = [];
...
function chooseRandomSpawnTiles(map = MAP_01) {
  const openTiles = shuffle(getAllOpenTiles(map));
  if (openTiles.length < 2) {
    return {
      p1: { row: map.startP1.row, col: map.startP1.col },
      p2: { row: map.startP2.row, col: map.startP2.col },
    };
  }
...
function loadMap(map) {
  ...
  const spawns = chooseRandomSpawnTiles(map);
  state.players[1].row = spawns.p1.row;
  state.players[1].col = spawns.p1.col;
  state.players[2].row = spawns.p2.row;
  state.players[2].col = spawns.p2.col;
}
...
function getReachableTiles(player) {
  ...
  for (let row = 0; row < SIZE; row++) {
    for (let col = 0; col < SIZE; col++) {
      if (isWallTile(row, col)) continue;
      if (state.players[player].row === row && state.players[player].col === col) continue;
      if (opponent.connected && opponent.row === row && opponent.col === col) continue;
...
function renderTileEngine() {
  ...
  for (let row = 0; row < SIZE; row++) {
    for (let col = 0; col < SIZE; col++) {
      ...
      cell.style.left = `${col * (TILE_SIZE + TILE_GAP)}px`;
      cell.style.top = `${row * (TILE_SIZE + TILE_GAP)}px`;
