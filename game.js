...
function getCrateClusterClasses(row, col) {
  const isCrate = (r, c) =>
    inBounds(r, c) && state.board[r][c].blocked && state.board[r][c].blockType === "crate";

  const left = isCrate(row, col - 1);
  const right = isCrate(row, col + 1);
  const up = isCrate(row - 1, col);
  const down = isCrate(row + 1, col);

  const classes = [];
  if (left || right) classes.push("crate-h");
  if (up || down) classes.push("crate-v");
  if (!left && right) classes.push("crate-open-left");
  if (left && !right) classes.push("crate-open-right");
  if (!up && down) classes.push("crate-open-top");
  if (up && !down) classes.push("crate-open-bottom");
  if (left && right) classes.push("crate-mid-h");
  if (up && down) classes.push("crate-mid-v");
  return classes.join(" ");
}
  ...
  for (let row = 0; row < SIZE; row++) {
    for (let col = 0; col < SIZE; col++) {
      ...
      if (cellData.blocked) {
        ...
        let extra = cellData.blockType === "crate" ? getCrateClusterClasses(row, col) : "";
...
async function maybeFinishAfterMove(player) {
  ...
  if (state.movesRemaining <= 0 && manhattan(mePos, oppPos) === 1) {
    await finishRound(player);
    return true;
  }
  return false;
