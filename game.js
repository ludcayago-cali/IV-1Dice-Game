...
function createBaseBoard() {
  const board = Array.from({ length: SIZE }, (_, row) =>
    Array.from({ length: SIZE }, (_, col) => ({
      ...
      col,
      blocked: false,
      ground: (row + col) % 2 === 0 ? "grass1" : "grass2",
      blockType: null,
    }))
  );
...
function slimeMarkup(colorClass, extraClass = "") {
  return `<div class="slime ${colorClass} ${extraClass}"></div>`;
}

function groundStyle(groundKey) {
  if (groundKey === "grass2") {
    return "background: radial-gradient(circle at 30% 25%, #aee86a 0 6%, transparent 7%), radial-gradient(circle at 72% 72%, #77c442 0 7%, transparent 8%), linear-gradient(135deg,#7a6550 0%,#9a8269 32%,#8c775f 56%,#6d5a48 100%);";
  }
  return "background: radial-gradient(circle at 24% 24%, #98dd61 0 6%, transparent 7%), radial-gradient(circle at 75% 77%, #5fac3e 0 6%, transparent 7%), linear-gradient(135deg,#84705b 0%,#aa9074 32%,#988169 56%,#74604e 100%);";
}

function blockStyle(type) {
  if (type === "bush") {
    return "background: radial-gradient(circle at 50% 42%, #7ed85c 0%, #3f8e31 58%, #214d1d 100%); border-radius: 16px;";
  }
  return "background: repeating-linear-gradient(0deg,#8a5b2f 0 12px,#6d431d 12px 14px), repeating-linear-gradient(90deg,#8a5b2f 0 12px,#6d431d 12px 14px); box-shadow: inset 0 0 0 3px rgba(50,25,10,.45);";
}
...
function renderTileEngine() {
  ...
  for (let row = 0; row < SIZE; row++) {
    for (let col = 0; col < SIZE; col++) {
      ...
      ground.style = groundStyle(cellData.ground);
      ...
      if (cellData.blocked) {
        ...
        block.style = blockStyle(cellData.blockType);
      ...
      for (const player of [1, 2]) {
        if (state.players[player].row === row && state.players[player].col === col) {
          ...
          actor.innerHTML = slimeMarkup(player === 1 ? "blue" : "red", "cell-slime");
...
function renderUI() {
  ...
  if (state.phase === "roll") {
    phaseText.textContent =
      state.currentPlayer === 1 ? "Click Roll Dice to begin." : "Bot is thinking...";
  } else if (state.phase === "move") {
    phaseText.textContent =
      state.currentPlayer === 1 ? "Click a highlighted tile to move." : "Bot is moving...";
  } else {
...
async function finishRound(winner) {
  ...
  resultPopupSubtext.textContent = matchOver ? "Series decided." : "Ready for the next round.";
...
document.addEventListener("keydown", (e) => {
  ...
  const nextRow = state.players[1].row + dr;
  const nextCol = state.players[1].col + dc;
