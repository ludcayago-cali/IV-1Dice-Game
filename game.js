...
function getPlayer1Sprite() {
  const anim = TILESET.actor.p1[state.anim[1]] ? state.anim[1] : "idle";
  const facing = state.facing[1] === "left" ? "left" : "right";
...
function getSameBlockNeighbors(row, col, type) {
  ...
  return {
    ...
    left: isSame(row, col - 1),
    right: isSame(row, col + 1),
  };
}

function getAutoBlockScale(row, col, type) {
  const n = getSameBlockNeighbors(row, col, type);
  const count = [n.up, n.down, n.left, n.right].filter(Boolean).length;
  ...
  return { scale: 1, width: 1, height: 1, y: 0 };
}
...
function startRound(resetScores = false) {
  ...
  state.winner = null;
  state.locked = false;
  state.combatFx.lunge = false;
  state.combatFx.targetShake = false;
  state.anim[1] = "idle";
  state.anim[2] = "idle";
  updateFacing(1);
  updateFacing(2);
  ...
  for (let row = 0; row < SIZE; row++) {
    for (let col = 0; col < SIZE; col++) {
      ...
      if (cellData.blocked) {
        ...
        if (cellData.blockType === "crate") {
          extra = getCrateStackType(row, col);
        }

        const auto = getAutoBlockScale(row, col, cellData.blockType);
        ...
        block.style.setProperty("--block-scale", auto.scale);
        block.style.setProperty("--block-width", `${auto.width * 100}%`);
        block.style.setProperty("--block-height", `${auto.height * 100}%`);
        block.style.setProperty("--block-y", `${auto.y}px`);
        cell.appendChild(block);
      }

      if (state.players[1].row === row && state.players[1].col === col) {
        const actor = document.createElement("div");
        actor.className = `layer actor p1 ${state.combatFx.lunge ? "attack-lunge" : ""} ${state.facing[1] === "left" ? "face-left" : "face-right"}`.trim();
        actor.style.backgroundImage = `url('${getPlayer1Sprite()}')`;
        cell.appendChild(actor);
      }

      if (state.players[2].row === row && state.players[2].col === col) {
        const actor = document.createElement("div");
        actor.className = `layer actor p2 ${state.combatFx.targetShake ? "target-hit" : ""}`.trim();
...
function renderUI() {
  ...
  diceText.textContent = state.dice == null
    ? "Dice: -"
    : `Dice: ${state.dice} | Planned: ${state.planningPath.length}/${state.movesRemaining + state.planningPath.length}`;
  ...
  rollBtn.disabled = !(state.phase === "roll" && state.currentPlayer === 1 && !state.locked);
...
function getPlanningOrigin() {
  return state.planningPath.length ? state.planningPath[state.planningPath.length - 1] : state.players[1];
}

function refreshPlanningMoves() {
  if (!(state.phase === "move" && state.currentPlayer === 1) || state.locked || state.movesRemaining <= 0) {
    state.validMoves = [];
    return;
  }
...
async function finishRound(winner) {
  ...
  if (winner === 1) {
    ...
    state.anim[1] = "hit";
    state.combatFx.lunge = true;
    renderAll();
    await delay(260);

    state.combatFx.targetShake = true;
    renderAll();
    await delay(520);
...
async function runBotTurn() {
  await delay(350);
  if (state.phase !== "roll" || state.currentPlayer !== 2 || state.locked) return;
...
rollBtn.addEventListener("click", () => {
  if (!(state.phase === "roll" && state.currentPlayer === 1) || state.locked) return;
  rollDiceForPlayer(1);
});

endTurnBtn.addEventListener("click", async () => {
  if (!(state.phase === "move" && state.currentPlayer === 1 && state.movesRemaining === 0)) return;
  if (state.planningPath.length) await executePlannedPath(1);
  else nextTurn();
});
...
document.addEventListener("keydown", (e) => {
  if (!(state.phase === "move" && state.currentPlayer === 1) || state.locked) return;
