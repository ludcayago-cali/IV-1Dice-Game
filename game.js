  ...
  for (let attempt = 0; attempt < 1200; attempt++) {
    ...
    for (let i = 0; i < shuffled.length; i++) {
      for (let j = i + 1; j < shuffled.length; j++) {
        const a = shuffled[i];
        const b = shuffled[j];
        if (manhattan(a, b) >= 6) { p1 = { ...a }; p2 = { ...b }; break; }
      }
      if (p1 && p2) break;
    }
    if (!p1 || !p2) { p1 = { ...shuffled[0] }; p2 = { ...shuffled[1] }; }
...
function getCrateStackType(row, col) {
  const isCrate = (r, c) => inBounds(r, c) && state.board[r][c].blocked && state.board[r][c].blockType === "crate";
...
function renderUI() {
  ...
  if (state.phase === "roll") {
    ...
    phaseText.textContent = state.currentPlayer === 1
      ? (planning ? `Plan route: ${state.planningPath.length}/${state.dice} step(s)` : "Tap adjacent tiles to plan your route.")
      : "Bot is moving...";
  } else {
...
function chooseBotMove() {
  ...
  for (const move of moves) {
    ...
    if (state.prevPositions[2] && state.prevPositions[2].row === move.row && state.prevPositions[2].col === move.col) {
      score -= 180;
    }
